...
function Idles() {
	...
	const deleteIdle = (id) => {
		Requests.idle.delete(id).then(() => {
			Requests.idle.getAll().then((idle) => {
				setData([...idle]);
			});
		});
	};
						...
						<div key={el.id}>
							<Grid item>
								<ButtonBase sx={{ width: 192, height: 192 }}>
									<Img
										alt='complex'
										src={`http://localhost:5001/file?location=http%3A%2F%2Flocalhost%3A3000%2Fidle%2F${el.id}`}
									/>
								</ButtonBase>
							</Grid>
